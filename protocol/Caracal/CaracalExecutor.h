...
template <class Workload> class CaracalExecutor: public Worker{
public:
	...
	CaracalExecutor(std::size_t coordinator_id, std::size_t id, DatabaseType &db,
	...
	for (;;) {
		...
		n_started_workers.fetch_add(1);
      	...
      	while (static_cast<ExecutorStatus>(worker_status.load()) ==
             ExecutorStatus::Caracal_Analysis) {
        std::this_thread::yield();
      	}

		n_started_workers.fetch_add(1);
      	insert_write_sets(); // initialization phase
      	n_complete_workers.fetch_add(1);
		// wait to initialize
      	while (static_cast<ExecutorStatus>(worker_status.load()) ==
             ExecutorStatus::Caracal_Initializtion) {
        	process_request();
      	}

		n_started_workers.fetch_add(1);
      	run_transactions(); // execution phase
      	n_complete_workers.fetch_add(1);
		// wait to execute
		while (static_cast<ExecutorStatus>(worker_status.load()) ==
             ExecutorStatus::Caracal_Execution) {
        	process_request();
      	}
  ...
  void setup_execute_handlers(TransactionType &txn) {
	txn.read_handler = [this, &txn](CaracalRWKey &readKey, std::size_t tid,
                                    uint32_t key_offset) {
  ...
  void insert_write_sets() {
	for (auto i = 0u; i < transactions.size(); i++) {
      ...
      std::vector<CaracalRWKey> &writeSet = t.writeSet;
